...
int noclip_unlocked = 0; // Флаг для разблокировки режима noclip
...
const char *noclip_code = "1234"; // Код для разблокировки noclip
...
typedef struct Node {
    Point point;
    int g, h; // g - стоимость пути, h - эвристическая стоимость
    struct Node *parent;
} Node;
...
void generate_map() {
    ...
    pacman_x = 1;
    pacman_y = 1;
    // Если в стартовой позиции была точка, уменьшаем общее число монет на 1
    if (map[pacman_y][pacman_x] == '.') {
        total_points--;
    }
    map[pacman_y][pacman_x] = ' '; // Начальная позиция Пакмана

    // Гарантируем, что клетка справа от Пакмана свободна (для движения вправо)
    if (pacman_x + 1 < WIDTH - 1 && map[pacman_y][pacman_x + 1] == '#') {
...
void draw_map() {
    ...
    mvprintw(HEIGHT, 0, "Score: %d/%d", score, total_points);
    mvprintw(HEIGHT + 1, 0, "Noclip: %s", noclip_unlocked ? (noclip ? "ON" : "OFF") : "LOCKED");
...
void move_pacman(int dx, int dy) {
    ...
    if (noclip || map[new_y][new_x] != '#') { // Проверяем, чтобы Пакман не сталкивался со стеной
        if (map[new_y][new_x] == '.') { // Если Пакман съел точку
            map[new_y][new_x] = ' ';
            score++;
            // Если все точки собраны — конец игры
...
void reconstruct_path(Node* node) {
    while (node->parent != NULL) {
        move_pacman(node->point.x - node->parent->point.x,
...
Point find_nearest_point() {
    ...
    for (int y = 0; y < HEIGHT; y++) {
        for (int x = 0; x < WIDTH; x++) {
            if (map[y][x] == '.') {
                int distance = heuristic((Point){pacman_x, pacman_y}, (Point){x, y});
...
void auto_move() {
    ...
    Point target_point = find_nearest_point();
    if (target_point.x == -1 && target_point.y == -1) return;
    ...
    while (open_list.size > 0) {
        ...
        if (current->point.x == target_point.x &&
            current->point.y == target_point.y) {
            reconstruct_path(current);
            return;
        }

        for (int i = 0; i < 4; i++) {
            Point neighbor_point = { current->point.x + directions[i].x,
                                       current->point.y + directions[i].y };
            if (map[neighbor_point.y][neighbor_point.x] == '#' ||
                find_node(&closed_list, neighbor_point) != NULL)
                continue;
            ...
            if (neighbor == NULL) {
                ...
                neighbor->h = heuristic(neighbor_point, target_point);
...
void check_code() {
    if (strcmp(input_code, correct_code) == 0) {
        ...
        mvprintw((HEIGHT / 2) + 1, (WIDTH / 2) - 12, "Entered code: %s", input_code);
        refresh();
        getch();
        endwin();
        exit(0);
        ...
        noclip_unlocked = 1;
        mvprintw(HEIGHT + 3, 0, "Noclip unlocked! Use 'K' to toggle.");
        refresh();
    }
    input_index = 0;
    input_code[0] = '\0';
