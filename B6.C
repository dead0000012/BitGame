...
const char *noclip_code = "1234"; // Код для включения/выключения noclip
...
void check_code() {
    if (strcmp(input_code, correct_code) == 0) {
        ...
        noclip = !noclip; // Переключаем noclip
        mvprintw(HEIGHT + 3, 0, "Noclip mode %s!", noclip ? "activated" : "deactivated");
        refresh();
    }
    input_index = 0;
    input_code[0] = '\0';
