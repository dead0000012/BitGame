...
void auto_move();
void check_code();
int is_reachable(Point start, Point end);
...
void generate_map() {
    ...
    } while (!is_reachable((Point){pacman_x, pacman_y}, find_nearest_point()));
}
...
void reconstruct_path(Node* node) {
    while (node->parent != NULL) {
        ...
        node = node->parent;
        draw_map();
        usleep(50000); // Уменьшаем задержку для ускорения автопрохождения
    }
}
...
void auto_move() {
    ...
    if (target_point.x == -1 && target_point.y == -1) return; // Нет доступных точек
    ...
    while (open_list.size > 0) {
        ...
        if (current->point.x == target_point.x && current->point.y == target_point.y) {
...
int is_reachable(Point start, Point end) {
    ...
    NodeList queue = {.size = 0};
    Node* start_node = malloc(sizeof(Node));
    start_node->point = start;
    add_node(&queue, start_node);
    ...
    while (queue.size > 0) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (nx >= 0 && nx < WIDTH && ny >= 0 && ny < HEIGHT && map[ny][nx] != '#' && !visited[ny][nx]) {
                if (nx == end.x && ny == end.y) return 1;

                visited[ny][nx] = 1;
                Node* neighbor = malloc(sizeof(Node));
                neighbor->point = (Point){nx, ny};
                add_node(&queue, neighbor);
            }
        }
